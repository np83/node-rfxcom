 ...
 */
function Lighting2(rfxcom : IRfxCom, subtype : number) {
    var self: ILighting2 = this;
 ...
 * correct length.
 */
Lighting2.prototype._splitDeviceId = function (deviceId: string) {
    var self : ILighting2 = this;
    var parts = deviceId.split("/");
    if (parts.length !== 2) {
        throw new Error("Invalid deviceId format, should be address/unit");
    }
    var idBytes = self.rfxcom.stringToBytes(parts[0]);
    if (idBytes.length !== 4) {
 ...
 */
(<ILighting2>Lighting2.prototype).switchOn = function(deviceId, callback) {
    return (<ILighting2>this)._sendCommand(deviceId, 1, undefined, callback);
};

/*
 * Switch off deviceId/unitCode
 */
(<ILighting2>Lighting2.prototype).switchOff = function(deviceId, callback) {
    return (<ILighting2>this)._sendCommand(deviceId, 0, undefined, callback);
};

/*
 * Switch off deviceId/unitCode
 */
(<ILighting2>Lighting2.prototype).setLevel = function(deviceId, level, callback) {
    if ((level < 0) || (level > 0xf)) {
        throw new Error("Invalid level value must be in range 0-15.");
    }
    return (<ILighting2>this)._sendCommand(deviceId, 2, level, callback);
};
