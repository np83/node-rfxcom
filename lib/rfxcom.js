...
var serialport = require("serialport"), rfxcom = require("./index"), EventEmitter = require("events").EventEmitter, util = require("util");
...
RfxCom.prototype.open = function () {
    ...
    self.serialport.on("data", function (data) {
        ...
        var length = data.shift(), packetType = data.shift(), handler = self.handlers[packetType];
...
RfxCom.prototype.messageHandler = function (data) {
    var self = this, subtype = data[0], seqnbr = data[1], responses = {
        0: "ACK - transmit OK",
...
RfxCom.prototype.statusHandler = function (data) {
    var self = this, receiverTypes = {
        ...
        0x5A: "868.35MHz FSK",
        0x5B: "868.95MHz"
    }, subtype = data[0], seqnbr = data[1], cmnd = data[2], msg = data.slice(2, 13), receiverType = receiverTypes[msg[1]], firmwareVersion = msg[2], protocols = [];
...
RfxCom.prototype.sendMessage = function (type, subtype, cmd, extra, callback) {
    var self = this;
    var cmdId = self.getCmdNumber();
    var byteCount = extra.length + 4;
    var buffer = [byteCount, type, subtype, cmdId, cmd];
...
RfxCom.prototype.enable = function (protocols, callback) {
    var self = this;
    var msg = [0x53, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
...
RfxCom.prototype.save = function (callback) {
    var self = this;
    var msg = [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
...
RfxCom.prototype.elec23Handler = function (data) {
    var self = this;
    var TOTAL_DIVISOR = 223.666, subtype = data[0], seqnbr = data[1], id = "0x" + self.dumpHex(data.slice(2, 4), false).join(""), count = data[4], instant = data.slice(5, 9), total = data.slice(9, 15), currentWatts = self.bytesToUint32(instant), totalWatts = self.bytesToUint48(total) / TOTAL_DIVISOR, roundedTotal = Math.round(totalWatts * Math.pow(10, 2)) / Math.pow(10, 2), evt = {
        subtype: subtype,
        id: id,
        count: count,
...
RfxCom.prototype.security1Handler = function (data) {
    var self = this;
    var subtype = data[0], seqnbr = data[1], id = "0x" + self.dumpHex(data.slice(2, 5), false).join(""), deviceStatus = data[5] & ~0x80, batterySignalLevel = data[6], evt = {
        subtype: subtype,
        id: id,
...
RfxCom.prototype.temp19Handler = function (data) {
    var self = this;
    var subtype = data[0], seqnbr = data[1], id = "0x" + self.dumpHex(data.slice(2, 4), false).join(""), temperature = ((data[4] & 0x7f) * 256 + data[5]) / 10, signbit = data[4] & 0x80, batterySignalLevel = data[6], evt = {
        subtype: subtype,
        id: id,
        seqnbr: seqnbr,
...
RfxCom.prototype.temphumidity19Handler = function (data) {
    var self = this;
    var subtype = data[0], seqnbr = data[1], id = "0x" + self.dumpHex(data.slice(2, 4), false).join(""), temperature = ((data[4] & 0x7f) * 256 + data[5]) / 10, signbit = data[4] & 0x80, humidity = data[6], humidityStatus = data[7], batterySignalLevel = data[8], evt = {
        subtype: subtype,
        id: id,
        seqnbr: seqnbr,
        temperature: temperature * (signbit ? -1 : 1),
...
RfxCom.prototype.lighting1Handler = function (data) {
    var self = this, subtypes = {
        ...
        6: "All On",
        7: "Chime"
    }, subtype = subtypes[data[0]], seqnbr = data[1], housecode = String.fromCharCode(data[2]), unitcode = data[3], command = commands[data[4]], rssi = data[5] >> 4, id = self.dumpHex(data.slice(2, 4), false).join(""), evt;
    evt = {
        id: "0x" + id,
        subtype: subtype,
        seqnbr: seqnbr,
...
RfxCom.prototype.lighting2Handler = function (data) {
    ...
    var commands = {
        ...
        4: "Group On",
        5: "Set Group Level"
    }, subtype = data[0], seqnbr = data[1], idBytes = data.slice(2, 6), unitcode = data[6], command = commands[data[7]], level = data[8], rssi = (data[9] & 0xf0) >> 4, evt;
...
RfxCom.prototype.lighting5Handler = function (data) {
    ...
    var subtypes = {
        ...
        6: "Mood4",
        7: "Mood5"
    }, subtype = subtypes[data[0]], seqnbr = data[1], id = "0x" + self.dumpHex(data.slice(2, 5), false).join(""), unitcode = data[5], command = commands[data[6]], evt = {
        subtype: subtype,
        id: id,
        unitcode: unitcode,
        command: command,
...
RfxCom.prototype.rfxmeterHandler = function (data) {
    var self = this, id = "0x" + self.dumpHex(data.slice(2, 4), false).join(""), counter = self.dumpHex(data.slice(4, 8), false).join(""), evt = {
        subtype: data[0],
        id: id,
        seqnbr: data[1],
...
RfxCom.prototype.weightHandler = function (data) {
    var self = this, id = "0x" + self.dumpHex(data.slice(2, 4), false).join(""), batterySignalLevel = data[6], evt = {
        subtype: data[0],
        id: id,
        seqnbr: data[1],
...
RfxCom.prototype.rfxsensorHandler = function (data) {
    ...
    var subtype = data[0];
    var seqnbr = data[1];
    var id = "0x" + self.dumpHex([data[2]], false).join("");
    var evt = {
        subtype: subtype,
        id: id,
        seqnbr: seqnbr,
...
RfxCom.prototype.tempbaro12Handler = function (data) {
    var self = this, id = "0x" + self.dumpHex(data.slice(2, 4), false).join(""), temperature = ((data[4] & 0x7f) * 256 + data[5]) / 10, signbit = data[4] & 0x80, barometer = ((data[8] & 0x7f) * 256 + data[9]), batterySignalLevel = data[11], evt = {
        subtype: data[0],
        id: id,
        seqnbr: data[1],
        temperature: temperature * (signbit ? -1 : 1),
...
//# sourceMappingURL=rfxcom.js.map